...
const Assignment = () => {
  ...
  if (!isLoading && !isError && assignments.length > 0)
    content = assignments?.map((assignment) => {
      const { title, id, video_title, totalMark } = assignment || {};
      return (
        <tr key={id}>
          ...
          <td className="table-td flex gap-x-2">
            <svg
              onClick={() => handleAssignmentDelete(id)}
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
              stroke="currentColor"
  ...
  const handleAddClick = () => {
    navigate("/admin/assignment/add-assignment");
  };
        ...
        <div className="mx-auto max-w-full px-5 lg:px-20">
          <div className="px-3 py-20 bg-opacity-10">
            <div className="w-full flex">
              <button className="btn ml-auto" onClick={handleAddClick}>
                Add Assignment
              </button>
            </div>
